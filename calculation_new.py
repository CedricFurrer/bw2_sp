...
import bw2data
import pandas as pd
from bw2calc.errors import NonsquareTechnosphere, EmptyBiosphere
# from matrix_utils.errors import EmptyArray
import helper as hp

# !!! Where is the EmptyArray error stored?
...
class LCA_Calculation():
    ...
    def __init__(self,
        ...
        check_activities = [m for m in activities if not isinstance(m, bw2data.backends.proxies.Activity)]
        ...
        # Add to object
        self.activities: list[bw2data.backends.proxies.Activity] = activities
    ...
    def load_Brightway_mappings(self) -> None:
        ...
        for db_name in bw2data.databases:
            ...
            for m in bw2data.Database(db_name):
                ...
                self.key_to_id_mapping[_key]: dict = _id
                self.id_to_key_mapping[_id]: dict = _key
                self.key_to_dict_mapping[_key]: dict = _instance
                self.id_to_dict_mapping[_id]: dict = _instance
                self.key_to_activity_mapping[_key]: bw2data.backends.proxies.Activity = m
                self.id_to_activity_mapping[_id]: bw2data.backends.proxies.Activity = m
    ...
    def get_production_amount(self,
                              activity: bw2data.backends.proxies.Activity) -> float:
    ...
    def extract_non_production_exchanges(self,
                                         activity: bw2data.backends.proxies.Activity) -> list[dict]:
    ...
    def retrieve_activity_key_data_as_dictionary(self, activity: bw2data.backends.proxies.Activity) -> dict:
    ...
    def extract_LCI_exchanges_of_activity_at_certain_level(self,
                                                           activity: bw2data.backends.proxies.Activity,
                                                           level: int) -> list[dict]:
        ...
        _temp_exchange_keys_to_calculate: dict = {current_level: (activity.key,)}
        
        # Initialize a new dictionary to store the compiled exchanges in case we need to retrieve them again throughout the process
        _temp_compiled_exchange_at_level: dict = {current_level: {}}
        
        # Extract exchanges as long/as many times as there are levels
        while current_level <= level:

            # Extract all the relevant keys at the current level
            exc_keys: tuple = _temp_exchange_keys_to_calculate[current_level]
            
            # Initialize a new list for the current level
            _temp_exchange_keys_to_calculate[current_level + 1]: tuple = ()
            
            # Initialize a new dictionary for the current level
            _temp_compiled_exchange_at_level[current_level]: dict = {}
            
            # Intialize variable to store the exchange results at the current level in case to return afterwards
            exchanges_at_level: list = []
            
            # Loop through each exchange key
            for exc_key in exc_keys:
                
                # First get the activity corresponding to the current exchange key
                exc_as_act: bw2data.backends.proxies.Activity = self.key_to_activity_mapping[exc_key]
                
                # Extract all non production exchanges of the activity
                non_production_exchanges: list[dict] = self.extract_non_production_exchanges(exc_as_act)
                
                # If no production exchanges are found, this means we have a biosphere flow that needs to be added
                if non_production_exchanges == []:
                    
                    # Return empty list if we are at level 1 because apparently the activity does not have any starting exchange
                    if current_level == 1:
                        return []
                    
                    # We get the compiled exchange from the iteration before
                    compiled_from_before: dict = _temp_compiled_exchange_at_level[current_level - 1][exc_key]
                    
                    # And we add this biosphere flow to the results
                    exchanges_at_level += [compiled_from_before]
                    
                    # We can then continue with the next exchange
                    continue
                
                # Get the production amount
                production_amount: (float | int) = self.get_production_amount(exc_as_act)
                
                # Add "flow" string to the start of the dictionary keys, add the production amount of the activity as well as the current level
                non_production_exchanges_str_added: list[dict] = [{**self.add_str_to_dict_keys(m, self.key_name_flow),
                                                                   self.key_name_production_amount: production_amount,
                                                                   self.key_name_functional_amount: self.functional_amount,
                                                                   self.key_name_flow + "_amount": (self.functional_amount / production_amount * m["amount"]) if production_amount != 0 else 0,
                                                                   self.key_name_level: current_level} for m in non_production_exchanges]
                ...
                # Merge all relevant information together in a dictionary to be returned
                compiled: list[dict] = [{**self.retrieve_activity_key_data_as_dictionary(activity),
                                         **m,
                                         **activity_info_str_added} for m in non_production_exchanges_str_added]
                
                # Add exchanges to the results
                exchanges_at_level += compiled
                
                # Add relevant activity key to the list in order to calculate it in the next round
                _temp_exchange_keys_to_calculate[current_level + 1] += tuple([m["input"] for m in non_production_exchanges])
                
                # Convert the list of dictionaries to a dictionary with 'key' as keys
                # Add this dictionary to the temporary dictionary
                _temp_compiled_exchange_at_level[current_level] |= {m[self.key_name_flow + "_input"]: m for m in compiled}
    ...
    def _initialize_calculation(self) -> None:
        ...
        for database in bw2data.databases:
            ...
            _act: bw2data.backends.proxies.Activity = [m for m in bw2data.Database(database)][0]
        ...
        for database in bw2data.databases:
            ...
            for method in self.methods:
                ...
                try:
                    ...
                    self.characterized_activity_dicts[(database, method)]: bw2calc.dictionary_manager.ReversibleRemappableDictionary = copy.deepcopy(lca_object.dicts.activity)
                    self.characterized_biosphere_dicts[(database, method)]: bw2calc.dictionary_manager.ReversibleRemappableDictionary = copy.deepcopy(lca_object.dicts.biosphere)
                    
                    # Create a pandas Series of the mapped dicts
                    self.characterized_activity_series[(database, method)]: pd.Series = pd.Series(pd.DataFrame(self.characterized_activity_dicts[(database, method)])[0].map(self.id_to_dict_mapping), name = "flows")
                    self.characterized_biosphere_series[(database, method)]: pd.Series = pd.Series(pd.DataFrame(self.characterized_biosphere_dicts[(database, method)])[0].map(self.id_to_dict_mapping), name = "flows")
    ...
    def _get_uncharacterized_contribution_dicts(self) -> None:
        ...
        for database in bw2data.databases:
                ...
                try:
                    ...
                    # Extract the biosphere and activity dictionaries
                    self.uncharacterized_activity_dicts[database]: bw2calc.dictionary_manager.ReversibleRemappableDictionary = copy.deepcopy(lca_object.dicts.activity)
                    self.uncharacterized_biosphere_dicts[database]: bw2calc.dictionary_manager.ReversibleRemappableDictionary = copy.deepcopy(lca_object.dicts.biosphere)
                    
                    # Create a pandas Series of the mapped dicts
                    self.uncharacterized_activity_series[database]: pd.Series = pd.Series(pd.DataFrame(self.uncharacterized_activity_dicts[database])[0].map(self.id_to_dict_mapping), name = "flows")
                    self.uncharacterized_biosphere_series[database]: pd.Series = pd.Series(pd.DataFrame(self.uncharacterized_biosphere_dicts[database])[0].map(self.id_to_dict_mapping), name = "flows")
    ...
    # Important to calculate the LCI process and emission contribution
    def _get_characterized_inventory_matrices(self, activity: bw2data.backends.proxies.Activity) -> None:
        ...
        for method in self.methods:
            ...
            if self.characterized_inventory.get((activity.key, method)) is None:
                ...
                _redo.redo_lci({activity.id: self.functional_amount})
                _redo.lcia()
    ...
    # Important to calculate the LCI process and emission contribution
    def _get_uncharacterized_inventory_matrices(self, activity: bw2data.backends.proxies.Activity) -> None:
    ...
    def calculate_LCIA_score(self,
        ...
        activity: bw2data.backends.proxies.Activity = self.key_to_activity_mapping[activity_key]
    ...
    def retrieve_CFs(self) -> None:
        ...
        loaded_methods: dict = {(m[0], m[1]): [(n[0].key, n[1]) for n in bw2data.Method(m[0])] for m in list(metadata.keys())}
