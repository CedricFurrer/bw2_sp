...
# from variables import (starting,
#                        STRIP,
#                        CASE_INSENSITIVE,
#                        REMOVE_SPECIAL_CHARACTERS,
#                        BIOSPHERE_NAME,
#                        BIOSPHERE_NAME_UNLINKED,
#                        TECHNOSPHERE_FIELDS,
#                        BIOSPHERE_FIELDS_I,
#                        BIOSPHERE_DB_NAME_MAPPING,
#                        BIOSPHERE_NAME_ABB,
...
#                        SALCA_ABB,
#                        WFLDB_ABB,
#                        REG_ADDITIONAL_NAME_FRAGMENT_ABB,
#                        SIMAPRO_DB_NAME_MAPPING_TECH,
#                        SIMAPRO_DB_NAME_MAPPING_TECH_REG,
#                        XML_DB_NAME_MAPPING_TECH,
#                        XML_DB_NAME_MAPPING_TECH_REG)
...
#                 unique_unlinked[exc.get("type")].add(hp.format_values(tuple([exc[m] for m in BIOSPHERE_FIELDS_I]),
#                                                                       case_insensitive = CASE_INSENSITIVE,
#                                                                       strip = STRIP,
#                                                                       remove_special_characters = REMOVE_SPECIAL_CHARACTERS))
                
#             else:
#                 unique_unlinked[exc.get("type")].add(hp.format_values(tuple([exc[m] for m in TECHNOSPHERE_FIELDS]),
#                                                                       case_insensitive = CASE_INSENSITIVE,
#                                                                       strip = STRIP,
#                                                                       remove_special_characters = REMOVE_SPECIAL_CHARACTERS))
...
#                                              biosphere_name: str = BIOSPHERE_NAME,
#                                              fields: list | tuple = BIOSPHERE_FIELDS_I,
...
#         biosphere_unlinked_loaded = {(m["database"], m["code"]): dict(m.as_dict(), **{"exchanges": []}) for m in copy.deepcopy(bw2data.Database(biosphere_new_name))}
        
#         # Write list of existing biosphere flows, but only the fields extracted
#         existing = [hp.format_values(tuple([v[m] for m in fields]),
#                                      case_insensitive = CASE_INSENSITIVE,
#                                      strip = STRIP,
#                                      remove_special_characters = REMOVE_SPECIAL_CHARACTERS) for _, v in biosphere_unlinked_loaded.items()]
...
#         if biosphere_new_name in bw2data.databases:
#             raise ValueError("The database '" + biosphere_new_name + "' (provided as variable 'biosphere_new_name') is already registered as database. Either use another name or switch variable 'add_to_existing_database' to True")
...
#     unique_unlinked_biosphere_flows = {hp.format_values(tuple([exc[m] for m in fields]),
#                                                         case_insensitive = CASE_INSENSITIVE,
#                                                         strip = STRIP,
#                                                         remove_special_characters = REMOVE_SPECIAL_CHARACTERS): exc for exc in unlinked_biosphere_flows}
...
#     if len(list(bw2data.Database(biosphere_name))) == 0:
#         raise ValueError("Biosphere database '" + str(biosphere_name) + "' not existing or empty")
    
#     # Extract all keys that should be kept based on an already existing, random biosphere flow
#     keep_keys = list(bw2data.Database(biosphere_name).random().as_dict().keys())
...
#         # Print statement
#         if verbose:
#             print(starting + "Delete database first: " + biosphere_new_name)
        
#         # Delete database
#         del bw2data.databases[biosphere_new_name]
...
#     # Print statement
#     if verbose:
#         print(starting + "Write database: " + biosphere_new_name)
    
#     # Register the new database
#     bw2data.Database(biosphere_new_name).register()
        
#     # Write the new flows to the new biosphere
#     bw2data.Database(biosphere_new_name).write(biosphere_new_data)
    
#     # ... and link again
#     db.apply_strategy(partial(bw2io.utils.ExchangeLinker.link_iterable_by_fields,
#                               other = (m for m in bw2data.Database(biosphere_new_name)),
#                               fields = fields,
#                               internal = False,
#                               kind = {"biosphere"},
#                               relink = False
