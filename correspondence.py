...
# Path where correspondence files are stored
path_correspondence_files = local_input_path / "Correspondence-Files"

# Pattern to extract the version names from the excel correspondence files
correspondence_file_pattern = ("^Correspondence\-File\-v"
                               "(?P<FROM_version>[0-9\.]+)"
                               "\-v"
                               "(?P<TO_version>[0-9\.]+)"
                               "(\.xlsx)$"
...
# Apply regex pattern
version_list_orig = [(k.name, re.match(correspondence_file_pattern, k.name)) for k in (path_correspondence_files).rglob("*.xlsx")]

# Convert version strings to tuples and sort the list so that the highest version is coming first
version_list = sorted([dict({k: tuple(int(n) for n in v.split(".")) for k, v in m.groupdict().items()}, **{"file": a}) for a, m in version_list_orig if m is not None], key = lambda x: x["TO_version"], reverse = False)

# all_sheet_names = [n for m in version_list for n in pd.read_excel(path_correspondence_files / m["file"], sheet_name = None)]
all_headers = []

# Names of sheets in the excel file depending on the ecoinvent model type
sheet_name_mapping = {"cutoff": ["Cut-off", "Cut-Off", "cut-off"],
                     "apos": ["apos", "APOS"],
                     "consequential": ["consequential", "Consequential", "conseq"]}

# Sheet names to search for in the excel files
sheetnames = sheet_name_mapping[XML_MODEL_TYPE]
...
for item in version_list:
    ...
    FROM_version_str = ".".join([str(m) for m in FROM_version])
    TO_version_str = ".".join([str(m) for m in TO_version])
    
    # Read the correspondence excel file for the current versions
    excel = pd.read_excel(path_correspondence_files / file, sheet_name = None)
    ...
    sheet = [m for m in sheetnames if m in excel]
    ...
    new_header_idx = [idx for idx, m in df_orig.iterrows() if not m["_header_selection"]][0]
    
    # We extract the new header based on the location found beforehand
    new_header_orig = list(df_orig.loc[new_header_idx])
    
    # We identify the duplicated headers
    doubles = [m for m in new_header_orig if new_header_orig.count(m) > 1]
    ...
    df = df_orig.copy().fillna("").iloc[new_header_idx+1:len(df_orig)]
    ...
    def find_colnames(possible_cols: list, raise_error_if_no_columns_were_found: bool = True) -> (str | None, str | None):
        ...
        if raise_error_if_no_columns_were_found:
            ...
            assert len(found) > 0, "No columns found. Used possible columns '" + str(possible_cols) + "'. Error occured in file '" + str(file) + "'" 
    ...
    colname_mult_orig = [m for m in mult_cols if m in new_header]
    
    # Raise error if the multiplier column was not found
    assert len(colname_mult_orig) > 0, "No multiplier column found. Used possible columns '" + str(mult_cols) + "'. Error occured in file '" + str(file) + "'"
    
    # Use the first element as column name for multiplier
    colname_mult = colname_mult_orig[0]
    ...
    for idx, row in df.iterrows():
        ...
        assert ID not in all_data, "Key is already present. Error occured for key '" + str(ID) + "' when reading file '" + str(file) + "'"
...
standardized = pd.DataFrame(list(all_data.values()))

# Write dataframe
standardized.to_excel(local_output_path / "standardized_correspondence_files.xlsx")


#%% Create a mapping file based on standardized correspondence files
...
pd.DataFrame(successfully_mapped + unsuccessfully_mapped).to_excel(local_output_path / "correspondence_file_mapped.xlsx")
